<?php
	...
	*/
	...
	$userToken = md5($_SESSION['login_token']);
	...
	sendResponseCodeAndExitIfTrue($userToken !== $_POST['logintoken'], 422); //Check if POST login token is correct
	
	$tryUserName = $_POST['user'];
	$tryUserPass = $_POST['pass'];
	
	$mysqlConn = connectToDatabase();
	$matchingUsers = getArrayFromSQLQuery($mysqlConn, 'SELECT userId, password, nick, role FROM users WHERE LOWER(nick) = LOWER(?) LIMIT 2', 's', [$tryUserName]);
